...
- [x] T006 [US1] Mount `actionsRouter` in `backend/src/routes/v1/index.ts` — add `import { actionsRouter } from './actions.js'` and `v1.route('/actions', actionsRouter)` after existing route registrations
- [x] T007 [P] [US1] Rewrite `app/actions/page.tsx` — `"use client"`, `useAuth()` + `apiClient`; `useEffect` fetching `GET /api/v1/actions`; group templates by platform (meta/google/shopify) as separate sections; each template card shows platform badge (blue=meta/orange=google/green=shopify), action_type badge, name, description, link to `/actions/:id`; loading state with `Loader2`; empty state if no actions; use design tokens (font-sans headings, font-body text, bg-white cards, shadow-sm, rounded-2xl)
...
- [x] T010 [US2] Mount `historyRouter` in `backend/src/routes/v1/index.ts` — add `import { historyRouter } from './history.js'` and `v1.route('/history', historyRouter)`
...
- [x] T017 Mark all tasks T001–T016 complete in `specs/004-execution-layer/tasks.md` (replace `- [ ]` with `- [x]` for all tasks)
