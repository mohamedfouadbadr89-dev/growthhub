...
- [x] T001 Install `openai@^4` package in `backend/` (`npm install openai`)
- [x] T002 [P] Add `OPENROUTER_API_KEY` and `OPENROUTER_DEFAULT_MODEL=google/gemini-2.0-flash-001` to `backend/.env.example`
...
- [x] T019 [P] Add `OPENROUTER_API_KEY=sk-or-xxx` to `backend/.env.example` (if not already present from T002)
- [x] T020 Mark all tasks complete in `specs/003-intelligence-layer/tasks.md` (replace `- [ ]` with `- [x]` for all T001–T019)
