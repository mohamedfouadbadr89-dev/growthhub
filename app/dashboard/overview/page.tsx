...
const campaigns = [
  { name: "Spring Collection 2024", platform: "Meta Ads", platformColor: "#1877F2", platformLetter: "F", spend: "$12,450", revenue: "$52,290", roas: "4.2x", roasType: "great", status: "Active", statusType: "active" },
  { name: "Brand Search - EMEA",    platform: "Google Ads", platformColor: "#EA4335", platformLetter: "G", spend: "$4,200",  revenue: "$18,480", roas: "4.4x", roasType: "great", status: "Active", statusType: "active" },
  { name: "UGC Influencer Push",    platform: "YouTube",   platformColor: "#FF0000", platformLetter: "Y", spend: "$8,900",  revenue: "$22,250", roas: "2.5x", roasType: "neutral", status: "Paused", statusType: "paused" },
  { name: "Retargeting Phase 2",    platform: "LinkedIn",  platformColor: "#0077B5", platformLetter: "L", spend: "$1,200",  revenue: "$1,440",  roas: "1.2x", roasType: "bad",    status: "Warning", statusType: "warning" },
];

const chartBars = [{ revenue: 40, spend: 32 }, { revenue: 55, spend: 47 }, { revenue: 45, spend: 34 }, { revenue: 70, spend: 63 }, { revenue: 60, spend: 49 }, { revenue: 85, spend: 81 }, { revenue: 75, spend: 66 }];
...
export default function DashboardOverview() {
  ...
  useEffect(() => {
    const fetchMetrics = async () => {
      ...
      try {
        ...
        if (data.spend === 0 && data.revenue === 0) setNoData(true);
        setMetrics(data);
      } catch {
        setNoData(true);
      } finally {
  ...
  const kpis = metrics
    ? [
        { label: "Revenue",     value: formatCurrency(metrics.revenue),     change: "30 days", positive: null as null, progress: Math.min(100, (metrics.revenue / 200000) * 100) },
        { label: "Spend",       value: formatCurrency(metrics.spend),        change: "30 days", positive: null as null, progress: Math.min(100, (metrics.spend / 80000) * 100) },
        { label: "ROAS",        value: metrics.roas.toFixed(2) + "x",       change: "", positive: metrics.roas >= 3 ? true : metrics.roas >= 1.5 ? null : false, progress: Math.min(100, (metrics.roas / 5) * 100) },
        { label: "Impressions", value: formatNumber(metrics.impressions),    change: "30 days", positive: null as null, progress: Math.min(100, (metrics.impressions / 10_000_000) * 100) },
        { label: "Clicks",      value: formatNumber(metrics.clicks),         change: "30 days", positive: null as null, progress: Math.min(100, (metrics.clicks / 100_000) * 100) },
      ]
    : [
        { label: "Revenue",     value: "—", change: "", positive: null as null, progress: 0 },
        { label: "Spend",       value: "—", change: "", positive: null as null, progress: 0 },
        { label: "ROAS",        value: "—", change: "", positive: null as null, progress: 0 },
        { label: "Impressions", value: "—", change: "", positive: null as null, progress: 0 },
        { label: "Clicks",      value: "—", change: "", positive: null as null, progress: 0 },
      ];
        ...
        {kpis.map((kpi) => (
          <div key={kpi.label} className="bg-white p-6 rounded-xl border border-border shadow-sm">
            ...
            <div className="flex items-baseline gap-2">
              <h4 className={`text-2xl font-sans font-extrabold tracking-tight text-foreground ${loading ? "animate-pulse" : ""}`}>
                {loading ? "…" : kpi.value}
              </h4>
              {kpi.change && (
                <span className={`text-xs font-bold ${kpi.positive === true ? "text-emerald-600" : kpi.positive === false ? "text-red-500" : "text-muted-foreground"}`}>
                  {kpi.change}
                </span>
              )}
            </div>
          ...
          <div className="flex-1 bg-white p-6 rounded-xl border border-border shadow-sm border-l-4 border-l-emerald-500">
            ...
            <ul className="space-y-4">
              <li className="flex gap-3">
                ...
                <p className="text-sm text-muted-foreground">Video Retargeting CTR increased by 24% following Creative Update A.</p>
              </li>
              <li className="flex gap-3">
                <span className="w-2 h-2 rounded-full bg-emerald-500 mt-1.5 shrink-0" />
                <p className="text-sm text-muted-foreground">Organic referral traffic from TechCrunch feature scaling well.</p>
              </li>
            </ul>
          </div>
          <div className="flex-1 bg-white p-6 rounded-xl border border-border shadow-sm border-l-4 border-l-red-500">
            ...
            <ul className="space-y-4">
              <li className="flex gap-3">
                ...
                <p className="text-sm text-muted-foreground">Landing Page B conversion dropped below 1.5% - Check mobile assets.</p>
              </li>
            </ul>
          </div>
        </div>
      ...
      <section className="bg-white rounded-xl border border-border shadow-sm overflow-hidden">
        <div className="p-8 border-b border-border flex justify-between items-center">
          <div>
            ...
            <p className="text-sm text-muted-foreground">Live breakdown of active ad sets across platforms</p>
          </div>
          <div className="flex gap-2">
            <button className="px-4 py-2 text-xs font-bold rounded-lg border border-border hover:bg-surface-container transition-colors">Export CSV</button>
                ...
                <tr key={c.name} className="hover:bg-surface-container-low transition-colors group">
                  ...
                  <td className="px-4 py-5">
                    <div className="flex items-center gap-2 text-sm">
                      <div className="w-6 h-6 rounded-full flex items-center justify-center text-[10px] text-white font-bold" style={{ backgroundColor: c.platformColor }}>{c.platformLetter}</div>
                  ...
                  <td className="px-4 py-5">
                    <span className={`px-2 py-1 rounded text-xs font-bold ${c.roasType === "great" ? "bg-emerald-100 text-emerald-800" : c.roasType === "bad" ? "bg-red-100 text-red-800" : "bg-secondary-container text-on-secondary-container"}`}>
                      {c.roas}
                    </span>
                  </td>
                  <td className="px-4 py-5">
                    <div className="flex items-center gap-2">
                      <span className={`w-2 h-2 rounded-full ${c.statusType === "active" ? "bg-emerald-500 animate-pulse" : c.statusType === "warning" ? "bg-red-500" : "bg-muted-foreground/30"}`} />
                      <span className="text-xs font-medium text-foreground">{c.status}</span>
                    </div>
                  </td>
                  <td className="px-8 py-5 text-right">
                    <button className="text-muted-foreground group-hover:text-primary transition-colors text-lg">⋮</button>
                  </td>
                </tr>
              ))}
            </tbody>
        ...
        <div className="p-4 bg-surface-container-low/30 flex justify-center">
          <button className="text-xs font-bold text-primary hover:underline transition-all">Load More Records</button>
        </div>
      </section>
    </div>
  );
